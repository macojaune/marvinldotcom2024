...
export const VoteResults = ({isPlaying, onPlayToggle}: {
			...
			?.map((project, index) => {
				...
				return (
				  <div
					...
					className="mb-2 flex flex-row flex-wrap items-center gap-4 py-2 border-b dark:border-d/primary border-l/primary"
				  >
					  <div className="text-2xl font-bold dark:text-d/primary">
                        #{index + 1}
				  </div>
					  <div className="flex flex-col">
						  <h3 className="text-xl font-bold dark:text-d/primary text-nowrap">
							  {project.title}
							  <span className="ml-2 px-1 font-semibold dark:bg-d/tertiary dark:text-d/secondary">
									  {project.votes.length} votes
								  </span>
						  </h3>
						  <p className="text-base dark:text-d/primary">{project.description}</p>

					  </div>
					  <PitchPlayer project={project} isPlaying={isPlaying === rootIndex}
...
export const ProjectBracket: React.FC<{ projects: Project[], activeProjectId: number }> = ({
	...
	return (
	  <QueryClientProvider client={queryClient}>
		  <>
			  {showResults ? (
				<VoteResults isPlaying={playingProject} onPlayToggle={handlePlayToggle}/>
			  ) : (
				<div className="mx-auto my-4">
					<h2
					  className="mb-2 text-2xl font-bold px-2 w-fit dark:bg-d/secondary dark:text-d/primary text-l/primary">
					...
					<div className="mb-8 flex flex-col sm:flex-row items-stretch justify-between sm:space-x-4">
						<ProjectCard
						  project={project1}
						...
						/>
						<div
						  className="my-4 sm:my-0 text-xl self-center font-bold dark:text-d/primary text-l/primary">vs.
						</div>
						<ProjectCard
						  project={project2}
