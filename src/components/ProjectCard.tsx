...
const ProjectCard: FC<{
	...
	isPlaying: boolean;
	onPlayToggle: () => void;
}> = ({project, onVote, isPlaying, onPlayToggle, isCurrent = false, className = ""}) => {
	if (!project) return null;
	return (
	  <div
		className={twMerge(
		  "w-full my-2 flex flex-col justify-stretch gap-4 border border-l/primary dark:border-d/primary p-4",
		  ...
		  >
			  ...
			  {!isCurrent && (
				<>
					...
					<PitchPlayer project={project} isPlaying={isPlaying} onPlayToggle={onPlayToggle}/>
				</>
			  )}
		  </div>
		  {!isCurrent && (
			<button
			  ...
			  className="mt-auto rounded border border-l/primary dark:border-d/primary px-4 py-2 text-white transition-all dark:hover:border-d/tertiary dark:hover:bg-d/tertiary hover:border-l/tertiary hover:bg-l/tertiary bg-l/primary dark:bg-transparent"
			>
				Voter
			</button>
		  )}
